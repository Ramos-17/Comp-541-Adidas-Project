...
def run_experiment(target: str = "Profit",
    ...
    # Make sibling import work whether run from src/, repo root, or a notebook.
    here = Path(__file__).resolve().parent
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    try:
        from .data_preprocessing import load_data, data_cleaning  # type: ignore # noqa: E402
    except ImportError:
        from data_preprocessing import load_data, data_cleaning  # type: ignore # noqa: E402
...
def train_full_lstm(target: str = "Profit",
                    window: int = 10,
                    hidden_units: int = 50,
                    dropout: float = 0.20,
                    l2_coeff: float = 0.01,
                    ...
                    verbose: int = 0) -> Dict:
    """
    Train the LSTM on the full cleaned daily series for export/deployment.

    This keeps the same architecture and training settings used in the notebook,
    but fits on the complete available time series so the exported artifact uses
    all historical data after the model-selection step is complete.
    """
    here = Path(__file__).resolve().parent
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    ...
    model, history, scaler = train_lstm(
        daily,
        target=target,
        window=window,
